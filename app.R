#########################################################################
#                               MangoShiny                              #
#          an app in R Shiny to visualize hospital management data      #
...
ui <- navbarPage(
  ...
  #set tab title
  windowTitle = "Hospital Management Data in R Shiny",
  #define menu
  navbarMenu(title = "Menu",
    ...
    tabPanel("Key Performance Indicators",
             ...
             #draw kpi figures
             plotOutput("val_rat", height = 400),
             plotOutput("pats", height = 400),
             plotOutput("cmi", height = 400),
             plotOutput("los", height = 400)
    ),
...
server <- function(input, output, session){
  ...
  output$map_legend <- renderText({
    "Index hospital (white pin) catchment area (purple) for neurosurgical patients, color intensity increases with admissions. Car travelling isochrones (CTI; 30, 60 minutes) to index hospital (green), color intensity decreases with travelling time. Not visible by default: Selection of neighbouring hospitals with equal or higher numbers of beds and levels of service (black circles), and respective CTIs (30, 60 minutes; blue). Expand checkboxes in the upper right corner of the map to toggle layer visibility."
  })
  ...
  output$val_rat <- renderPlot({
    plot(
      ...
      xlab = ("Months after Service Resumption"), ylab = "Reimbursed Valuation Ratios (Per Cent of Baseline)", xlim = c(k1(), k2()),
  ...
  output$cmi <- renderPlot({
    plot(kpi_data$month, 26*kpi_data$val_ratios/kpi_data$patients, xlab = ("Months after Service Resumption"), ylab = "Case Mix Index (Per Cent of Baseline)", xlim = c(k1(), k2()), col = "#440154FF", lwd = 4, type = "l")
  })
  output$los <- renderPlot({
    plot(kpi_data$month, kpi_data$mean_los, xlab = ("Months after Service Resumption"), ylab = "Mean Length of Stay (Days from Baseline)", xlim = c(k1(), k2()), col = "#39568CFF", lwd = 4, type = "l")
  })
